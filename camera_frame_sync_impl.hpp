    ...
    const typename CameraFrameSync<CameraInfoV>::Base& camera)
{
  ...
  {
    recording_output_dir_ = MakeDefaultRecordingDir(camera.NameView());
  }
  ...
  const auto path = std::filesystem::path(recording_output_dir_) / "sync_imu.csv";
  recording_sync_csv_.open(path, std::ios::out | std::ios::trunc);
  if (!recording_sync_csv_.is_open())
  {
    XR_LOG_ERROR("CameraFrameSync: open sync recording failed %s",
                 path.string().c_str());
    ASSERT(false);
    return;
  }
  ...
  XR_LOG_PASS("CameraFrameSync: sync recording enabled dir=%s",
              recording_output_dir_.c_str());
}
